...
TC01_24
    ...
    ${permission}=          Open Session and Login    email=${E_DUMB}              password=${PW_DUMB}
    ${ASCII}=               Format Set Sentence       length=${25}    charset=ASCII
    ${sentence}=            Generate Sentence         sentence_length=${30}        format_set=Base
    ${future_time}=         Generate Time             time_set=future_time         time_period=${2}
    ${past_time}=           Generate Time             time_set=past_time           time_period=${5}
    ${payload}=             Data Validator            sentence_1=${sentence}       sentence_2=${ASCII}                    start_at=${past_time}    end_at=${future_time}
    ${response}=            POST Task                 task=${payload}[task]        description=${payload}[description]       start_at=${payload}[start_at]    
    ...                                               end_at=${payload}[end_at]    user_id=${643}    expected_status=201     auth=${permission}
    Positive Validation     ${payload}    ${response}
...
TC01_25
    ...
    ${permission}=          Open Session and Login    email=${E_DUMB}              password=${PW_DUMB}
    ${Unicode}=             Format Set Sentence       length=${25}    charset=Unicode
    ${sentence}=           Generate Sentence         sentence_length=${30}        format_set=Base
    ${future_time}=         Generate Time             time_set=future_time         time_period=${2}
    ${past_time}=           Generate Time             time_set=past_time           time_period=${5}
    ${payload}=             Data Validator            sentence_1=${sentence}       sentence_2=${Unicode}                    start_at=${past_time}    end_at=${future_time}
    ${response}=            POST Task                 task=${payload}[task]        description=${payload}[description]       start_at=${payload}[start_at]    
    ...                                               end_at=${payload}[end_at]    user_id=${643}    expected_status=201     auth=${permission}
    Positive Validation     ${payload}    ${response}
...
TC01_26
    ...
    ${permission}=          Open Session and Login    email=${E_DUMB}              password=${PW_DUMB}
    ${ISO8859}              Format Set Sentence       length=${25}    charset=ISO8859
    ${sentence}=           Generate Sentence         sentence_length=${30}        format_set=Base
    ${future_time}=         Generate Time             time_set=future_time         time_period=${2}
    ${past_time}=           Generate Time             time_set=past_time           time_period=${5}
    ${payload}=             Data Validator            sentence_1=${sentence}       sentence_2=${ISO8859}                    start_at=${past_time}    end_at=${future_time}
    ${response}=            POST Task                 task=${payload}[task]        description=${payload}[description]       start_at=${payload}[start_at]    
    ...                                               end_at=${payload}[end_at]    user_id=${643}    expected_status=201     auth=${permission}
    Positive Validation     ${payload}    ${response}
